...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (existingRecords.length > 0) {
            ...
            await db.query(
                `UPDATE policy SET terms = ? WHERE user_id = ?`,
                [JSON.stringify(terms), user_id]
            );
            return NextResponse.json({ success: true, message: 'Policy updated' }, { status: 200 });
        } else {
            // Insert new record
            await db.query(
                `INSERT INTO policy (user_id, terms) VALUES (?, ?)`,
                [user_id, JSON.stringify(terms)]
            );
            return NextResponse.json({ success: true, message: 'Policy created' }, { status: 201 });
        }
    } catch (error) {
        return NextResponse.json({
            success: false,
